 ...
 */
export const ALLOWED_ALIASES: ReadonlyArray<ModelAlias> = [
  "best",
  "fast",
  "cheap",
  "code",
...
const ALIAS_SET: ReadonlySet<string> = new Set(ALLOWED_ALIASES);
 ...
 */
export function assertAlias(model: string): asserts model is ModelAlias {
  if (ALIAS_SET.has(model)) return;
  ...
  if (looksRaw) {
    throw new Error(
      `[@thio/llm-client] Raw vendor ID "${model}" is not allowed. Use a LiteLLM alias instead — e.g. "claude-sonnet" / "fast" / "cheap". ` +
  ...
  throw new Error(
    `[@thio/llm-client] Unknown model "${model}". Allowed aliases: ${ALLOWED_ALIASES.join(", ")}.`,
  );
}
