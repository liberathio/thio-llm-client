...
export const UNSUPPORTED_SCHEMA_KEYWORDS: readonly string[] = [
  "minimum",
  "maximum",
  "exclusiveMinimum",
  "exclusiveMaximum",
 ...
 */
export function assertObjectRoot(schema: Record<string, unknown>): void {
  const type = schema.type;
  const isObject = type === "object" || (type === undefined && "properties" in schema);
  if (!isObject) {
    throw new Error(
      `[@thio/llm-client] outputConfig.format.schema root must be an object (got type ${JSON.stringify(type)}). ` +
...
export function prepareOutputConfig<T extends { format?: { type: string; schema: Record<string, unknown> } }>(
  config: T,
): T {
  if (!config.format || config.format.type !== "json_schema") return config;
